...
from rekuest.blok.parser import bsx
from rekuest.blok.validate import validate_blok


def test_bsx_parser_reports_line_column_and_source_context() -> None:
    malformed = "<Page>\n  <Header />\n  <Label>\n</Page>\n<Footer />"
    ...
    assert "Failed to parse BSX/XML at line 4, column" in message
...
def test_bsx_parser_reports_eof_with_previous_context() -> None:
    malformed = "<Page>\n  <Label>\n        "
...
def test_explicit_id_wins_and_roots_its_children() -> None:
    card = bsx('<Card id="ot2-monitor"><Button /></Card>')
...
def test_inner_text_is_rejected_rather_than_dropped() -> None:
    with pytest.raises(ValueError) as exc_info:
        bsx('<Badge className="x">Opentrons is getting hot!</Badge>')
...
def test_hash_prop_is_a_plain_static_value_outside_foreach() -> None:
    prop = bsx('<Page color="#fff" />').props[0]
...
def test_hash_prop_outside_foreach_declares_no_local() -> None:
    ...
    component = bsx('<Page color="#fff"><Text text="@fff" /></Page>')
...
def test_foreach_let_declares_the_loop_variable() -> None:
    let_prop = next(
        prop
        for prop in bsx('<foreach items="@dep.state.xs" let="#item" />').props
        if prop.key == "let"
    )
...
def test_validation_error_names_the_component_and_its_id() -> None:
    component = bsx('<Card><Row><Text text="@nope.state.x" /></Row></Card>')
...
def test_util_call_accepts_signed_numeric_literals() -> None:
    ...
    component = bsx('<Text value="@utils.math.multiply(-2, dep.state.x)" />')
...
def test_util_call_base_positionals_are_named() -> None:
    ...
    component = bsx('<Text v="@utils.gt(dep.state.x, 2)" />')
