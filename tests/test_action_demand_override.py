...
A declared protocol method becomes an action demand. By default that demand
...
def _dependency_of(cls: Any) -> AgentDependencyInput:
    ...
    # `bytes` is not a port kind of its own, so it is an object kept on the
    # agent's shelve -- which now has to be said rather than assumed.
    registry.register_as_memory_structure(bytes)
