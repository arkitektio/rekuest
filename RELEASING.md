...
`rekuest` ships as a PyPI package (`rekuest`). Versioning is automated
...
| --- | --- | --- |
| `main` | stable `X.Y.Z` | the default install (`pip install rekuest`) |
| `next` | prereleases `X.Y.Z-rc.N` | published as a **prerelease** — only reached via `pip install rekuest --pre` or an exact pin |
| `N.x` (e.g. `1.x`) | maintenance `X.Y.Z` | published stable for an older major |

PyPI marks `…-rc.N` versions as prereleases, so a plain
`pip install rekuest` never picks them up — `next` is a safe soak channel.
...
```sh
pip install rekuest --pre          # latest rc (or stable, whichever is newer)
pip install 'rekuest==2.0.0-rc.1'  # pin a specific rc
```

Stable consumers (`pip install rekuest`) are unaffected by the `next`
channel.
